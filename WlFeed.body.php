	...
	extends WlSyndicationFeed
{
	...
	function outEntry( WlSyndicationEntry $entry ) {
		...
		if ( $description ) {
			...
			$authorNames = array();
			foreach ( $entry->getAuthors() as $author ) {
				$authorNames[] = $this->authorName($author['name']);
			}
			$description = "<p>by:" . implode($authorNames, ",") . "</p>" . $description;
			
			echo Xml::element( 'description', null, $description);
		}
	...
	function authorName( $author ) {
		...
		if ( !isset( $authorSigCache[$author] ) ) {
			...
			$res = $dbr->select('user', array('user_name', 'user_real_name'), "user_name = '" . $author . "'");
			foreach( $res as $row ) {
				if(strlen(trim($row->user_real_name)) > 0) {
					$authorRealName = $row->user_real_name;
				}
			}
			$authorSigCache[$author] = $authorRealName;
		}
