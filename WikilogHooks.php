...
class WikilogHooks
{
	...
	static function ArticleEditUpdates( &$article, &$editInfo, $changed ) {
		...
		if ( $title->isTalkPage() ) {
			...
			// @jbernal if ( $wi->getItemTitle()->exists() ) {
			if ( is_object($wi->getItemTitle())&&$wi->getItemTitle()->exists() ) {
			...
			$minor, $watch, $sectionanchor, &$flags )
	{
		...
		if ( isset( $article->mExtWikilog ) && $article->mExtWikilog['signpub'] ) {
			...
			$txtDate = $t['date'];
			$txtUser = $t['user'];
			$txtUserRealName = $user->getRealName();
			if($txtUserRealName){
				$text = rtrim( $text ) . "\n{{wl-publish: {$txtDate} | {$txtUser}:{$txtUserRealName} }}\n";
			} else {
